...
class Assumptions(BaseModel):
    rate_of_return: Decimal = Field(
        ..., description="Annual rate of return (decimal), e.g., 0.05"
    )
    salary_growth: Decimal = Field(
        ..., description="Annual salary growth (decimal), e.g., 0.03"
    )
    contribution_rate: Decimal = Field(
        ..., description="Contribution rate (decimal), e.g., 0.08"
    )
