...
def project_dc_account(inp: DCProjectionInput) -> DCProjectionOutput:
    ...
    for y in range(1, years + 1):
        ...
        balance = Money((balance.value * (Decimal(1) + rate)).quantize(Money("0.01").value))
        ...
        salary = Money((salary.value * (Decimal(1) + salary_growth)).quantize(Money("0.01").value))
...
def early_retirement_adjustment(annual_pension: Decimal, years_early: int, pct_per_year: Decimal = Decimal("0.05")) -> Decimal:
    base = Decimal(annual_pension)
    factor = (Decimal(1) - pct_per_year) ** Decimal(years_early)
    adj = (base * factor).quantize(Money("0.01").value)
    return adj

def late_retirement_adjustment(annual_pension: Decimal, years_late: int, pct_per_year: Decimal = Decimal("0.02")) -> Decimal:
    base = Decimal(annual_pension)
