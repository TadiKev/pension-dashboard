...
class Command(BaseCommand):
    help = 'Seed synthetic data for staging (companies, members, accounts, transactions)'
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(options['companies']):
            ...
            AssumptionSet.objects.create(company=comp, name='Default', assumptions={'rate_of_return': '0.05'})
            for i in range(options['members-per-company']):
                ...
                for t in range(random.randint(1,5)):
                    Transaction.objects.create(
                        ...
                        transaction_type=random.choice(['contribution','payout','adjustment']),
                        source='seed'
                    )
